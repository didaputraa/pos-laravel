
<div class="col-md-3">
	<div class="info-box border-primary info-box-border">
		<div class="info-box-icon">
			<svg width="60px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" fill="#007BFF">
				<path d="M6 2C4.897 2 4 2.897 4 4L4 20C4 21.103 4.897 22 6 22L12.683594 22C12.387594 21.378 12.181078 20.707 12.080078 20L12.074219 20C12.026219 19.673 12 19.34 12 19C12 18.305 12.105922 17.634 12.294922 17L11.818359 17C11.244359 16.91 9.4121094 16.479469 9.4121094 13.855469L11.302734 13.855469C11.302734 15.563469 12.182406 15.492188 12.316406 15.492188C12.463406 15.492188 13.185547 15.465609 13.185547 14.349609C13.185547 12.120609 9.7265625 13.108578 9.7265625 9.7675781C9.7265625 7.4775781 11.623891 7.121125 11.962891 7.078125L11.962891 5.6503906L12.970703 5.6503906L12.970703 7.1054688C13.291703 7.1664687 15.087891 7.576125 15.087891 10.203125L13.197266 10.203125C13.197266 8.830125 12.614141 8.6152344 12.369141 8.6152344C12.235141 8.6152344 11.615234 8.6503437 11.615234 9.7773438C11.616234 11.512344 14.248188 11.242375 14.867188 13.359375C15.771187 12.696375 16.841 12.250031 18 12.082031L18 12.085938C18.034 12.080937 18.064656 12.078219 18.097656 12.074219C18.393656 12.035219 18.693 12 19 12C19.326 12 19.643031 12.029266 19.957031 12.072266L19.957031 3.7167969C19.980031 3.3877969 19.863141 3.0518594 19.619141 2.7558594C19.317141 2.3908594 18.733 2 18 2L6 2 z M 19 14C16.239 14 14 16.239 14 19C14 21.761 16.239 24 19 24C21.761 24 24 21.761 24 19C24 16.239 21.761 14 19 14 z M 18 16L20 16L20 18L22 18L22 20L20 20L20 22L18 22L18 20L16 20L16 18L18 18L18 16 z" fill="#007BFF" />
			</svg>
		</div>
		<div class="info-box-content">
			<span class="info-box-text">Order hari ini </span>
			<span class="info-box-number">{{ $orderan->count() }}</span>
		</div>
	</div>
</div>
<div class="col-md-3">
	<div class="info-box border-primary info-box-border">
		<div class="info-box-icon">
			<svg width="55px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 26 26" fill="#007BFF">
				<path d="M13 0C9.699219 0 7 2.101563 7 6C7 8.609375 8.214844 11.3125 10 12.8125L10 14.1875C10 14.789063 9.59375 15.304688 9.09375 15.40625C5.195313 16.605469 2 19.1875 2 20.6875L2 22.5C2 24.398438 6.898438 26 13 26C19.101563 26 24 24.398438 24 22.5L24 20.6875C24 19.289063 20.90625 16.605469 16.90625 15.40625C16.40625 15.304688 16 14.6875 16 14.1875L16 12.8125C17.785156 11.3125 19 8.609375 19 6C19 2.101563 16.300781 0 13 0Z" fill="#007BFF" />
			</svg>
		</div>
		<div class="info-box-content">
			<span class="info-box-text">Konsumen</span>
			<span class="info-box-number">{{ $konsumen }}</span>
		</div>
	</div>
</div>
<div class="col-md-3">
	<div class="info-box border-primary info-box-border">
		<div class="info-box-icon">
			<svg width="54px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 26 26" fill="#007BFF">
				<path d="M12.4375 0.1875L0 5.09375L0 18.53125L15 25.90625L26 18.53125L26 5.09375 Z M 12.46875 2.34375L23.6875 6.375L21.03125 7.78125L9.75 3.40625 Z M 7.03125 4.5L18.6875 9.03125L15.59375 10.65625L3.4375 5.90625 Z M 2.0625 6.4375L15 11.5L15 23.5L14.84375 23.59375L2 17.28125L2 6.46875 Z M 6.5 10.65625L5.03125 12.5625L6 12.875L6 15.625L7 16L7 13.21875L8 13.53125 Z M 10.5 12.03125L9 13.96875L10 14.3125L10 17.34375L11 17.71875L11 14.65625L11.96875 14.96875 Z M 5 16.03125L5 17.25L12 20.5625L12 19.3125Z" fill="#007BFF" />
			</svg>
		</div>
		<div class="info-box-content">
			<span class="info-box-text">Produk</span>
			<span class="info-box-number">{{ $produk }}</span>
		</div>
	</div>
</div>
<div class="col-md-3">
	<div class="info-box border-primary info-box-border">
		<div class="info-box-icon">
			<svg width="50px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" fill="#007BFF">
				<path d="M11 0L11 5L10 1C10 1 5 2.199219 5 7C5 7 4 7.199219 4 9L20 9C20 7.199219 19 7 19 7C19 2.199219 14 1 14 1L13 5L13 0 Z M 5 10C5 13.84375 8.15625 17 12 17C15.84375 17 19 13.84375 19 10L17 10C17 12.753906 14.753906 15 12 15C9.246094 15 7 12.753906 7 10 Z M 7.40625 17.40625C4.40625 18.207031 2.101563 20 1 24L23 24C21.898438 20 19.59375 18.207031 16.59375 17.40625C15.195313 18.105469 13.601563 18.59375 12 18.59375C10.300781 18.59375 8.804688 18.207031 7.40625 17.40625Z" fill="#007BFF" />
			</svg>
		</div>
		<div class="info-box-content">
			<span class="info-box-text">Pengerjaan</span>
			<span class="info-box-number">{{ $pengerjaan }}</span>
		</div>
	</div>
</div>

<div class="col-lg-12">
	<div class="card bg-white">
		<div class="card-header">
			<h6 class="card-title">Penjualan Tahun {{ date('Y',strtotime('now')) }}</h6>
		</div>
		<canvas id="grafik-penjualan" style="height:300px"></canvas>
		<div class="card-footer"></div>
	</div>
</div>

<div class="col-lg-4">
	<div class="info-box bg-success">
		<div class="info-box-icon">
		
		</div>
		<div class="info-box-content">
			<div class="info-box-text">Barang Masuk</div>
			<div class="info-box-number">{{ $barang_in }}</div>
		</div>
	</div>
	<div class="info-box bg-warning">
		<div class="info-box-icon">
		
		</div>
		<div class="info-box-content">
			<div class="info-box-text">Piutang</div>
			<div class="info-box-number">{{ $piutang }}</div>
		</div>
	</div>
</div>
<div class="col-lg-8">
	<div class="card card-primary">
		<div class="card-header">
			<h5 class="card-title">Orderan hari ini</h5>
		</div>
		<div class="card-body">
			<table class="table">
				<tr>
					<th>Konsumen</th>
					<th>jumlah</th>
					<th>total</th>
				</tr>
			@foreach($orderan->take(4)->get() as $r)
				<tr>
					<td>{{ $r->Nama }}</td>
					<td>{{ $r->jumlah }}</td>
					<td>{{ number_format($r->total,0, "," ,".") }}</td>
				</tr>
			@endforeach
			</table>
		</div>
		<div class="card-footer">
			<div class="float-right">Total <span id="total-order-now">{{ $orderan->count() }}</span></div>
		</div>
	</div>
</div>
<script>
function grafixCreate()
{
	let data1 = {
		labels: {!! $bulan !!},
		datasets:[
			{
				borderColor	: '#2c92ff',
				data		: {!! $bulanData !!},
				fill		: false
			}
		]
	};

	let grafix_penjualan = $('#grafik-penjualan').get(0).getContext('2d');

	new Chart(grafix_penjualan,{
		type:'line',
		data:data1,
		options:{
			scales:{
				xAxes:[{
					gridLines:{
						display:false
					}
				}]
			},
			legend:{
				display:false
			}
		}
	});
}
grafixCreate();
</script>